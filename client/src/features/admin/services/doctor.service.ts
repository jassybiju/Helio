...
export const adminDoctorService = {
  async getDoctors(params?: DoctorQueryParams) {
    return (await apiRequest(
      ...
      null,
      params,
    )) as APIResponse<{ doctors: Doctor[]; totalCount: number }>;
  },
  getDoctor(id: string) {
    return apiRequest(
      API_ENDPOINT.ADMIN.DOCTOR.GET(id),
      HTTP_METHOD.GET,
    ) as Promise<APIResponse<DoctorView>>;
  },
  toggleDoctor(userId: string) {
    return apiRequest(
      API_ENDPOINT.ADMIN.DOCTOR.TOGGLE(userId),
      HTTP_METHOD.PATCH,
    );
  },
  doctorApproval(
    ...
    }: { verification_status: DOCTOR_STATUS; rejection_reason?: string | null },
  ) {
    return apiRequest(
      API_ENDPOINT.ADMIN.DOCTOR.APPROVAL(userId),
      HTTP_METHOD.PATCH,
