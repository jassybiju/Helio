...
export const authService = {
  ...
  async verify_otp({ id, otp }: { id: string; otp: string }): Promise<unknown> {
    const response = await apiRequest("/patient/auth/verify-otp", "POST", {
      id,
      otp,
    });
    return response;
  ...
  async login({
    ...
    const response = await apiRequest("/patient/auth/login", HTTP_METHOD.POST, {
      email,
      password,
    });
    return response;
