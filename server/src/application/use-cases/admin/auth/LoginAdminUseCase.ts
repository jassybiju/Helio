import type { ILoginRequestDTO, ILoginResponseDTO } from "@application/dto/auth/ILoginDTO.ts";
...
export class LoginAdminUseCase implements ILoginUseCase {
  ...
  async execute(input: ILoginRequestDTO): Promise<ILoginResponseDTO> {
    ...
    const accessToken = this._accessTokenService.generateAccessToken(ADMIN_ID, ADMIN_EMAIL, USER_ROLES.ADMIN )
    const refreshToken = this._refreshTokenService.generateRefreshToken()
    
    //saving refresh token
    await this._sessionRepo.storeRefreshToken(ADMIN_ID, USER_ROLES.ADMIN, this._refreshTokenService.hash(refreshToken))
