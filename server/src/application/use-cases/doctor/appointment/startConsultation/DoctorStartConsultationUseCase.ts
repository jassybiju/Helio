...
export class DoctorStartConsultationUseCase implements IDoctorStartConsultationUseCase {
  ...
  async execute(
    ...
    return await this._uow.execute(async (session) => {
      ...
      const existingConsultation = await consultationRepo.findByAppointmentId(
        appointment.id
      );

      if (existingConsultation) {
        throw new ConflictError(
          "Consultation with the appointmentId already exists"
        );
      }

      // checking if doctor is currently doing an appointment
      const ongoingAppointment =
        await appointmentRepo.findOngoingAppointmentByDoctor(doctorId);
