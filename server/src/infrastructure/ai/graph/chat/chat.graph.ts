...
const ChatState = Annotation.Root({
  ...
  route: Annotation<"rag" | "findDoctor" | "chat" | "wallet">(),
});
...
export function createChatGraph({
  ...
  const graph = new StateGraph(ChatState)
    ...
    .addNode(
      "wallet",
      createWalletNode({ model, walletTool: tools.walletBalance })
    )
...
// Node 1: Fast Router
function createRouterNode({ model }: { model: BaseChatModel }) {
  ...
  return async (state: typeof ChatState.State) => {
    console.log(
      "🐛 [Node: router] Entering with message count:",
...
- "chat": Simple greetings ("hi", "hello") or off-topic chat.`),
    ...
    if (result.route === "chat") {
      return {
        ...
        messages: [
          new AIMessage(
            "Hello! How can I assist you with your health or finding a doctor today?"
          ),
        ],
      };
    }
...
function createRAGNode({
  ...
  return async (state: typeof ChatState.State) => {
    ...
    const response = await model.invoke([
      new SystemMessage(
        `Answer the patient using only this platform information:\n\n${JSON.stringify(docs)}
...
function createFindDoctorNode({
  model,
  tools,
}: {
  model: BaseChatModel;
  tools: CreateChatGraphOptions["tools"];
}) {
  const matchSpecialtyModel = model.withStructuredOutput(
    z.object({
      matchedSpecialty: z
        .string()
        .nullable()
        .describe(
          "The exact specialty string from the available list that matches the user's intent or typo. Return null if no match exists."
        ),
      isMatchFound: z
        .boolean()
  ...
  return async (state: typeof ChatState.State) => {
    ...
    console.log(
      "🐛 [Node: findDoctor] Platform Specialties retrieved:",
      platformSpecialties
    );
    debugger;
...
${JSON.stringify(platformSpecialties)}`),
    ...
    console.log(
      "🐛 [Node: findDoctor] LLM Specialty Match Result:",
      matchResult
    );
    debugger;
    ...
    if (!matchResult.isMatchFound || !matchResult.matchedSpecialty) {
      ...
      return {
        messages: [
          new AIMessage(
            "I couldn't find an available specialty on our platform that matches your query. Could you please specify your symptoms or the specialty you're looking for?"
          ),
        ],
      };
    }
    ...
    console.log(
      "🐛 [Node: findDoctor] Doctor Search Results for:",
      matchResult.matchedSpecialty,
      doctorResults
    );
    debugger;
