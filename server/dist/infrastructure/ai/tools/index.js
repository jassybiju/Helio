...
export function createGetSpecialtyTool(specialtyRepo) {
    return new DynamicStructuredTool({
        name: "get_specialities",
        description: "Get the list of medical specialties available on Helio. Call this before recommending a specialty to a patient based on their symptoms.",
        schema: z.object({}),
        func: async () => {
...
export function createSearchDoctorTool(doctorRepo) {
    return new DynamicStructuredTool({
        name: "search_doctors",
        description: "Search for doctors. `query` should contain the specialty and/or doctor name, e.g. 'Cardiology' or 'Cardiology, Dr. Ann'.",
        schema: z.object({
...
export const getDoctorDetailsTool = new DynamicStructuredTool({
    ...
    func: async ({ doctorId }) => {
        console.log("🔧 [Tool: get_doctor_details] Invoked with doctorId:", doctorId);
        debugger;
        const doctor = DOCTORS.find((d) => d.id === doctorId);
        if (!doctor)
            return { error: `Doctor ${doctorId} not found` };
        return { doctor };
    },
});
export const getDoctorSlotsTool = new DynamicStructuredTool({
    name: "get_doctor_slots",
    description: "Get available appointment slots (dateTime + consultationType) for a doctor.",
    schema: z.object({
        doctorId: z.string(),
    }),
    func: async ({ doctorId }) => {
...
export const bookAppointmentTool = new DynamicStructuredTool({
    ...
    description: "Book an appointment. Only call this after explicit confirmation.",
    schema: z.object({
        doctorId: z.string(),
        date: z.string().describe("ISO dateTime matching available slots"),
        consultationType: z.enum(["ONLINE", "CLINIC"]),
    }),
    func: async ({ doctorId, date, consultationType, }) => {
