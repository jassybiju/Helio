...
export const patientCompleteProfileSchema = z.object({
    gender: z.enum(["male", "female", "other"], {
        error: (issue) => issue.input === undefined ? "gender is required" : "Gender must be valid",
    }),
    dob: z
        .string()
        .refine((date) => !isNaN(Date.parse(date)), "Date of birth must be a valid date string")
        .refine((date) => new Date(date) < new Date(), "Date of birth must be before today"),
    phone: z
        .string()
