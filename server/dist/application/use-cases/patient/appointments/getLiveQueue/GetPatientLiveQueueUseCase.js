...
export class GetPatientLiveQueueUseCase {
    _logger;
    _patienRepo;
    _appointmentRepo;
    constructor(_logger, _patienRepo, _appointmentRepo) {
        this._logger = _logger;
        this._patienRepo = _patienRepo;
        this._appointmentRepo = _appointmentRepo;
    }
    async execute(appointmentId, patientId) {
        ...
        const queueAppointments = await this._appointmentRepo.findActiveQueueByDoctorAndTime(appointment.doctorId, new Date(appointment.startTime.getTime() + 60 * 1000));
        const userIndex = queueAppointments.findIndex((a) => a.id === appointment.id);
        const queueNumber = userIndex !== -1 ? userIndex : queueAppointments.length;
        // find which appointment is at the top
        const ongoingAppointment = queueAppointments.find((a) => a.status === APPOINTMENT_STATUS.ONGOING);
        ...
        if (!referenceAppointment) {
            ...
            if (!referenceAppointment) {
                throw new AppError("Doctor Is Started Consultation", HTTPStatus.BAD_REQUEST);
            }
        }
        const queueNumberOfOngoingAppointment = queueAppointments.indexOf(referenceAppointment);
        // this._logger.debug("referenceAppoint", {
        //   queueAppointments,
        //   referenceAppointment,
        // });
        const slotDurationMs = appointment.endTime.getTime() - appointment.startTime.getTime();
...
//# sourceMappingURL=GetPatientLiveQueueUseCase.js.map