...
import { combineDateAndTime, dayMap, istToUtc, } from "#shared/utils/date.utils.js";
export class SlotGenerator {
    constructor() { }
    generateSlots(shift, date) {
        const slots = [];
        ...
        while (currentTime.isBefore(shift.endTime)) {
            ...
            slots.push(new DoctorSlot(shift.shiftId, shift.doctorId, istToUtc(combineDateAndTime(date, currentTime)), istToUtc(combineDateAndTime(date, nextTime)), shift.consultationType, shift.capacityPerSlot, 0, shift.location));
            currentTime = nextTime;
        }
        return slots;
    }
    generateSlotsFromRange(shifts, startDate, endDate) {
        const slots = [];
    ...
    generateNextAvailableSlot(shifts, fromDate) {
        const result = [];
        const doctorShiftMap = new Map();
        ...
        for (let i = 0; i < MAX_DAYS; i++) {
            ...
            for (const [doctorId, doctorShifts] of doctorShiftMap.entries()) {
                if (completedDoctors.has(doctorId))
                    continue;
                const todayShifts = doctorShifts.filter((s) => dayMap[s.dayOfWeek] === day);
